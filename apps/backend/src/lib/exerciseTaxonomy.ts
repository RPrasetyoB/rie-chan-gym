import type { ExerciseSeed } from '../data/exercises.js'
...
function normalizeParts(parts: string[]) {
  return Array.from(new Set(parts)).sort(
    (left, right) => (BODY_PART_PRIORITY.get(left as (typeof BODY_PART_ORDER)[number]) ?? 999) - (BODY_PART_PRIORITY.get(right as (typeof BODY_PART_ORDER)[number]) ?? 999),
  )
}

export function deriveBodyParts(exercise: Pick<ExerciseSeed, 'category' | 'muscleGroups'>) {
